...
const props = withDefaults(
  defineProps<{
    ...
    disableClick?: boolean // Disable link opening if true
  }>(),
  {
    hue: 260,
    borderRadius: '5em',
...
function openGithub(e: MouseEvent) {
  if (props.disableClick) {
    // Do nothing, do not open link, do not prevent default or stop propagation
    return
  }
  e.preventDefault()
...
<template>
  ...
  >
    <button
      ...
      type="button"
      tabindex="0"
      :aria-disabled="props.disableClick ? 'true' : 'false'"
    >
...
.sparkles {
  ...
  transition:
    ...
    background-position 0.5s cubic-bezier(0.77, 0, 0.18, 1);
...
.sparkles:after {
  ...
  box-shadow:
    inset 0 0.25em 0.75em rgba(0, 0, 0, 1),
...
.sparkles .content {
  ...
  filter: drop-shadow(0 0 0.05em rgba(0, 0, 0, 0.5))
    drop-shadow(0 0.05em 0.05em rgba(0, 0, 0, 0.5));
